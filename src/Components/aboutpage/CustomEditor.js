...
export default class CustomEditor extends Component {
  constructor(props) {
    ...
    this.state = {
      editorValue: content
        ? createValueFromString(content, "html")
        : createEmptyValue(),
  ...
  componentWillReceiveProps(nextProps) {
    if (!this.props.content && nextProps.content) {
      this.setState({
        editorValue: nextProps.content
          ? createValueFromString(nextProps.content, "html")
          : createEmptyValue(),
      })
    }
    ...
    if (!this.props.saveBool && nextProps.saveBool) {
      this.saveContent(
        this.state.editorValue.toString("html"),
        this.props.contentLocation,
      )
      this.props.postSave()
    }
    ...
    if (!this.props.cancelBool && nextProps.cancelBool) {
      this.setState({
        editorValue: this.props.content
          ? createValueFromString(this.props.content, "html")
          : createEmptyValue(),
      })
        ...
        {this.props.inputHtmlBool ? (
          <div>
            ...
            <ButtonContainer>
              ...
              <Button onClick={() => this.convertHtml(this.state.textarea)}>
                Convert HTML
              </Button>
            </ButtonContainer>
          </div>
